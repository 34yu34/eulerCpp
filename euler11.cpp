...
int main(int argc, char const *argv[])
{
    ...
    const uint32_t count_size = 4;
    const uint32_t usable_size = line_size - count_size;

    uint64_t max = 0;
    for (uint32_t i = 0; i < data.size(); i++)
    {
        if (i % line_size >= usable_size || // check if its the end of a line
            i / line_size >= usable_size) // check if its the end of a column
        {
            continue;
        }

        uint64_t left = data[i] * data[i+1] * data[i+2] * data[i+3];
        uint64_t down = data[i] * data[i + line_size] * data[i + (2 * line_size)] + data[i + (3 * line_size)];
        uint64_t down_left = data[i] * data[i + 1 + line_size] * data[i + 2 * (1 + line_size)] * data[i + 3 * (1 + line_size)];
        max = Number::max(max, left);
        max = Number::max(max, down);
        max = Number::max(max, down_left);
    }
