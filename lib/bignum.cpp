...
ostream & operator<<(ostream & o, const Bignum & num)
{
  ...
  for (int i = num.data_size - 1; i >= 0; i--) {
    if (num.data[i] < 10 && i + 1 != (int)num.data_size) {
      o << "0";
    }
    o << (int)num.data[i];
  }
...
bool Bignum::operate(const Bignum & num, bool (*fptr)(int, int)) const
{
  if (data_size == num.data_size) {
    ...
    return fptr(data_size, num.data_size);
  }
}

int Bignum::find_sign(const Bignum & num) const
{
...
* Addition
*********************************************************************/
void Bignum::addition_step(uint8_t * result, const Bignum & num1, const Bignum & num2, uint8_t & ret, uint32_t index) const
{
  if (num1.data_size > index && num2.data_size > index) {
    *result = (num1.data[index] + num2.data[index] + ret) % BASE;
    ret = (num1.data[index] + num2.data[index] + ret) / BASE;
  }
  else if (num1.data_size > index) {
    *result = (num1.data[index] + ret) % BASE;
    ret = (num1.data[index] + ret) / BASE;
  }
  else if (num2.data_size > index) {
...
Bignum Bignum::operator+(const Bignum & num) const
{
  ...
  uint32_t maxSize = *this > num ? data_size + 1 : num.data_size + 1;
  uint8_t * newData = new uint8_t[maxSize];
  uint8_t ret = 0;
  for (uint32_t i = 0; i < maxSize; i++) {
    addition_step(newData + i, *this, num, ret, i);
  }
...
* Substraction
*********************************************************************/
void Bignum::substraction_step(uint8_t * result, const Bignum * max, const Bignum * min, uint8_t & stole, uint32_t index) const
{
  if (max->data_size > index && min->data_size > index) {
...
Bignum Bignum::operator-(const Bignum & num) const
{
  ...
  uint8_t * newData = new uint8_t[max->data_size];
  uint8_t stole = 0;

  for (uint32_t i = 0; i < max->data_size; i++) {
    substraction_step(newData + i, max, min, stole, i);
  }

  uint32_t size = chop(newData, max->data_size);
...
{
  int sign = find_sign(num);
  uint32_t size = data_size + num.data_size;
...
void Bignum::init_division(const Bignum & num, Bignum & quotient, Bignum & denominator, int & index) const
{
  index = 1;
  denominator = +num;
  quotient = +(*this);
...
void Bignum::divise_step(Bignum & reminder, Bignum & denominator, uint8_t * newData) const
{
  uint8_t diviser = 0;
  ...
  *newData = diviser;
  ...
  // divide by 100
  denominator = Bignum(denominator.data + 1, denominator.data_size - 1, false);
}

Bignum Bignum::operator/(const Bignum & num) const
{
  bool sign = find_sign(num) == 0;
  if (*this < num) return Bignum(0);

  int maxIndex;
  Bignum denominator, quotient;

  init_division(num, quotient, denominator, maxIndex);

  uint8_t * newData = new uint8_t[maxIndex];

  for (int i = maxIndex - 1; i >= 0; i--) {
    divise_step(quotient, denominator, newData + i);
  }
...
Bignum Bignum::operator%(const Bignum & num) const
{
  ...
  init_division(num, reminder, denominator, maxIndex);

  uint8_t * newData = new uint8_t[maxIndex];

  for (int i = maxIndex - 1; i >= 0; i--) {
    divise_step(reminder, denominator, newData + i);
  }
